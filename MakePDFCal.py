...
#   maximum width
#
def FindMaxFontHeight (pdfFile, Font, FontStyle, BaseHeight, MaxWidth, MaxTestString):
...
def MakePDFMonthCal (year, month, calParams, outputFile):
        ...
        pdfFile.set_auto_page_break(False)
        ...
        for calMonth in range (startMonth, endMonth+1):
                ...
                fontScaleFactor, fontMaxWidth, fontMaxSize = FindMaxFontHeight (pdfFile, calParams['Font'], fontStyle, \
                ...
                gridOffset = 0
                if (calParams['DayNumPlacement'] == 2 or calParams['DayNumPlacement'] == 3):
                ...
                for i in cal.itermonthdays (year, calMonth):
                        ...
                        if (i != 0):
                                ...
                                # Corner placement with chores
                                elif (calParams['DayNumPlacement'] == 3):
                                        ...
                                        if (calParams['ChoreFontBold'] == True):
                                                chorefontStyle += 'b'
                                        if (calParams['ChoreFontItalic'] == True):
                                                chorefontStyle += 'i'

                                        if calParams['ChoreFont'] != 'helvetica':
                                                pdfFile.add_font(calParams['ChoreFont'], chorefontStyle, f"font/{calParams['ChoreFont']}.ttf", uni=True)
                                                
                                        chorefontScaleFactor, chorefontMaxWidth, chorefontMaxSize = FindMaxFontHeight (pdfFile, calParams['ChoreFont'], chorefontStyle, \
                                                                                                                       calParams['BlockDayRegionHeight'] * calHeight / GRID_ROWS, \
                                                                                                                       calParams['DayChoreFontScale'] * calWidth / NUMBER_WEEKDAYS,
                                                                                                                       MAX_NUMBER_STR)

                                        numberXLoc += 0.1
                                        numberYLoc += 0.4
                                        if calParams['Debug']:
                                                print(f'DishPn numberLoc: {numberXLoc:.3f}, {numberYLoc:.3f}')
                                        ...
                                        numberYLoc += 0
                                        if calParams['Debug']:
                                                print(f'Dishes numberLoc: {numberXLoc:.3f}, {numberYLoc:.3f}')
